...
interface ParsedRow {
  ...
  type: 'income' | 'expense'; category_suggestion?: string
  status: 'ok' | 'error' | 'duplicate'; error?: string
...
// ── Componente de Export isolado (carrega react-pdf apenas client-side) ───────
function ExportTab() {
  ...
  // Componentes do react-pdf carregados dinamicamente
  const [PDFLink,  setPDFLink]  = useState<any>(null)
  const [PDFDoc,   setPDFDoc]   = useState<any>(null)
  const [pdfReady, setPdfReady] = useState(false)

  useEffect(() => {
    // Importa react-pdf apenas no cliente, após mount
    Promise.all([
      ...
      console.error('Erro ao carregar @react-pdf/renderer')
    })
  }, [])
  ...
  return (
    <div className="space-y-4">
      <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
        ...
        <p className="text-xs text-indigo-500">Gera um relatório completo com resumo executivo, despesas por categoria, saldo por conta e tabela de transações.</p>
      </div>
      ...
      {reportData && (
        <div className="bg-white border border-gray-100 rounded-xl p-5 space-y-4">
          <p className="text-sm font-semibold text-gray-700">Prévia — {reportData.period_label}</p>
          ...
          {/* Botão de download — só renderiza quando react-pdf estiver carregado */}
          {pdfReady && PDFLink && PDFDoc ? (
            ...
            <div className="w-full bg-gray-100 text-gray-400 rounded-xl py-3 text-sm text-center">
...
export default function ImportarPage() {
  ...
  function applyMapping() {
    ...
    const rows: ParsedRow[] = rawRows.map((row, i) => {
      const date = parseDate(row[mapping.date]??''); const amount = parseAmount(row[mapping.amount]??''); const desc = row[mapping.description]??''
  ...
  async function handlePDFFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setPdfText(fullText)
      const lines = fullText.split('\n').map(l=>l.trim()).filter(Boolean); const rows: ParsedRow[] = []
      for (const line of lines) {
        const match = line.match(/(\d{2}[\/\-]\d{2}[\/\-]\d{2,4})\s+(.+?)\s+([\-\+]?R?\$?\s?[\d.,]+)\s*$/)
        if (!match) continue
  ...
  function reset() {
    ...
    setPdfText(''); setResult(null); setFileName(''); setError('')
    if (fileRef.current) fileRef.current.value=''
    if (pdfRef.current)  pdfRef.current.value=''
  }

  const okRows = parsed.filter(r=>r.status==='ok'); const errorRows = parsed.filter(r=>r.status==='error')

  const DoneCard = ({ r }: { r: ImportResult }) => (
    <div className="space-y-4">
      <div className={`rounded-xl p-6 text-center border ${r.errors===0?'bg-green-50 border-green-100':'bg-amber-50 border-amber-100'}`}>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white rounded-xl p-3"><p className="text-xl font-bold text-green-600">{r.imported}</p><p className="text-xs text-gray-400">Importadas</p></div>
          <div className="bg-white rounded-xl p-3"><p className="text-xl font-bold text-amber-500">{r.duplicates}</p><p className="text-xs text-gray-400">Duplicatas</p></div>
          <div className="bg-white rounded-xl p-3"><p className="text-xl font-bold text-red-500">{r.errors}</p><p className="text-xs text-gray-400">Erros</p></div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 max-w-4xl mx-auto">
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {([{id:'csv',label:'📊 Importar CSV'},{id:'pdf',label:'📄 Importar PDF'},{id:'export',label:'📤 Exportar PDF'}] as const).map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); reset() }}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab===t.id?'bg-indigo-600 text-white':'bg-white border border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
            {t.label}
          </button>
        ))}
      </div>

      {/* Export — componente isolado que importa react-pdf apenas no cliente */}
        ...
        <>
          {step === 'upload' && (
            <div className="space-y-4">
              ...
              <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-gray-200 hover:border-indigo-400 rounded-xl p-10 text-center cursor-pointer transition-colors">
              ...
              <div className="bg-white border border-gray-100 rounded-xl p-4 flex items-center justify-between">
                <div><p className="text-sm font-medium text-gray-700">Não sabe o formato?</p><p className="text-xs text-gray-400 mt-0.5">Baixe o modelo padrão SaKel</p></div>
                <button onClick={() => { const csv='Data,Descricao,Valor\n01/05/2026,Supermercado,-150.50\n02/05/2026,Salario,5000.00'; const a=document.createElement('a'); a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(csv); a.download='modelo-sakel.csv'; a.click() }}
                  className="text-xs text-indigo-600 border border-indigo-200 px-3 py-1.5 rounded-lg hover:bg-indigo-50">Baixar modelo</button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[{key:'date',label:'Data *'},{key:'description',label:'Descrição *'},{key:'amount',label:'Valor *'}].map(f => (
                  <div key={f.key}>
                    ...
                    <select value={mapping[f.key as keyof typeof mapping]??''} onChange={e => setMapping({...mapping,[f.key]:e.target.value})}
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div className="space-y-4">
              ...
              {accounts.length > 0 && (
                <div className="bg-white border border-gray-100 rounded-xl p-4 flex items-center gap-4">
                  ...
                  <select value={selectedAcc} onChange={e => setSelectedAcc(e.target.value)} className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <option value="">Sem conta</option>{accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                  </select>
                </div>
              )}
              <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
                <div className="px-5 py-3 border-b border-gray-50 flex items-center justify-between"><p className="text-sm font-medium text-gray-700">Pré-visualização</p><p className="text-xs text-gray-400">{okRows.length} válidas</p></div>
                <div className="max-h-80 overflow-y-auto">
                  {parsed.slice(0,50).map((row,i) => (
                    <div key={i} className={`flex items-center gap-4 px-5 py-3 border-b border-gray-50 last:border-0 ${row.status==='error'?'bg-red-50':''}`}>
                      ...
                      <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-800 truncate">{row.description}</p><p className="text-xs text-gray-400">{row.date}{row.category_suggestion&&` · 🏷️ ${row.category_suggestion}`}{row.error&&` · ⚠️ ${row.error}`}</p></div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleImport} disabled={importing||okRows.length===0} className="flex-1 bg-indigo-600 text-white rounded-lg py-2.5 text-sm font-medium hover:bg-indigo-700 disabled:opacity-50">
      ...
      {tab === 'pdf' && (
        <div className="space-y-4">
          {step !== 'done' && (
            <>
              <div className="bg-amber-50 border border-amber-100 rounded-xl p-4"><p className="text-sm font-medium text-amber-700 mb-1">⚠️ Sobre importação de PDF</p><p className="text-xs text-amber-600">Funciona melhor com PDFs de texto (não escaneados).</p></div>
              <div onClick={() => pdfRef.current?.click()} className="border-2 border-dashed border-gray-200 hover:border-indigo-400 rounded-xl p-10 text-center cursor-pointer transition-colors">
              ...
              {pdfParsed.length>0&&(
                <>
                  ...
                  <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
                    <div className="px-5 py-3 border-b border-gray-50 flex items-center justify-between"><p className="text-sm font-medium text-gray-700">Transações detectadas</p><p className="text-xs text-gray-400">{pdfParsed.length} encontradas</p></div>
                    <div className="max-h-80 overflow-y-auto">
                      {pdfParsed.map((row,i)=>(
                        <div key={i} className="flex items-center gap-4 px-5 py-3 border-b border-gray-50 last:border-0">
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${row.type==='income'?'bg-green-50 text-green-600':'bg-red-50 text-red-500'}`}>{row.type==='income'?'↑':'↓'}</div>
                          <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-800 truncate">{row.description}</p><p className="text-xs text-gray-400">{row.date}{row.category_suggestion&&` · 🏷️ ${row.category_suggestion}`}</p></div>
                          <p className={`text-sm font-semibold shrink-0 ${row.type==='income'?'text-green-600':'text-red-500'}`}>{row.type==='income'?'+':'−'} {fmt(row.amount)}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  <button onClick={handlePDFImport} disabled={importing} className="w-full bg-indigo-600 text-white rounded-xl py-3 text-sm font-medium hover:bg-indigo-700 disabled:opacity-50">
                    {importing?'Importando…':`Importar ${pdfParsed.length} transações`}
                  </button>
                </>
              )}
              {pdfText&&pdfParsed.length===0&&!pdfLoading&&(
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 text-center"><p className="text-sm text-amber-700">Nenhuma transação detectada automaticamente.</p><p className="text-xs text-amber-500 mt-1">O PDF pode estar em formato de imagem ou layout não reconhecido.</p></div>
              )}
            </>
          )}
          {step==='done'&&result&&<DoneCard r={result}/>}
