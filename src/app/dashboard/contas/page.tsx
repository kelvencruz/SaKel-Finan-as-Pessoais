...
export default function ContasPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 max-w-4xl mx-auto">
      ...
      {/* Cartão de crédito? Link de atalho */}
      <div className="bg-purple-50 border border-purple-100 rounded-xl px-4 py-3 mb-6 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-purple-700">
          <span>💳</span>
          <span>Cartões de crédito são gerenciados separadamente.</span>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white border border-gray-100 rounded-xl p-5 sm:col-span-2">
          <p className="text-sm text-gray-500">Saldo total (contas)</p>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-dashed border-gray-200 rounded-xl p-10 text-center">
          ...
          <button onClick={openCreate} className="mt-3 text-indigo-600 text-sm hover:underline">Criar primeira conta</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {accounts.map(account => {
            ...
            return (
              <div key={account.id} className={`bg-white border rounded-xl p-5 flex items-start justify-between transition-opacity ${account.is_active ? 'border-gray-100' : 'border-gray-100 opacity-60'}`}>
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-full flex items-center justify-center text-white font-bold text-base shrink-0" style={{ backgroundColor: account.color }}>
                    {account.icon ? account.icon : account.name.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-medium text-gray-800">{account.name}</p>
                      {!account.is_active && <span className="text-[10px] bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded-full">inativa</span>}
                    </div>
                    <p className="text-xs text-gray-400">{info.emoji} {info.label}</p>
                    <p className={`text-sm font-semibold mt-1 ${Number(account.current_balance) >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                      {fmt(Number(account.current_balance))}
                    </p>
                  </div>
                </div>
                <div className="flex flex-col gap-1 items-end">
                  <button onClick={() => openEdit(account)} className="text-xs text-gray-400 hover:text-indigo-600 px-2 py-1 rounded hover:bg-indigo-50 transition-colors">Editar</button>
                  <button onClick={() => toggleActive(account)} className="text-xs text-gray-400 hover:text-amber-600 px-2 py-1 rounded hover:bg-amber-50 transition-colors">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
            <h2 className="text-lg font-semibold mb-5">{editingId ? 'Editar Conta' : 'Nova Conta'}</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Para cartões de crédito, use a seção <a href="/dashboard/cartoes" className="text-indigo-500 hover:underline">Cartões</a>.</p>
              </div>

              <div>
                <label className="block text-sm text-gray-600 mb-1">Ícone <span className="text-gray-400">(emoji opcional)</span></label>
                <input type="text" value={form.icon} onChange={e => setForm({ ...form, icon: e.target.value })}
                  placeholder="Ex: 💜 🏦 💰"
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" maxLength={4} />
              </div>
              ...
              {editingId && (
                <div className="flex items-center gap-3">
                  <input type="checkbox" id="is_active" checked={form.is_active} onChange={e => setForm({ ...form, is_active: e.target.checked })} className="w-4 h-4 accent-indigo-600" />
                  <label htmlFor="is_active" className="text-sm text-gray-600">Conta ativa</label>
                </div>
              )}
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="flex-1 border border-gray-200 text-gray-600 rounded-lg py-2 text-sm hover:bg-gray-50 transition-colors">Cancelar</button>
              <button onClick={handleSave} disabled={saving} className="flex-1 bg-indigo-600 text-white rounded-lg py-2 text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 transition-colors">
