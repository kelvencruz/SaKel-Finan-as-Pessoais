...
export default function Sidebar() {
  ...
  const NavLink = ({ item }: { item: { href: string; label: string; Icon: React.ElementType } }) => {
    ...
    return (
      <a
        ...
        className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium relative transition-colors"
        style={{
  ...
  const SidebarContent = () => (
    ...
    >
      {/* Logo */}
      <div className="px-5 py-3.5 mb-1.5 shrink-0" style={{ borderBottom: '1px solid var(--color-border)' }}>
        <img
      ...
      {gamEnabled && (
        <a
          ...
          onClick={() => setOpen(false)}
          className="mx-3 mb-2.5 px-3 py-2 rounded-xl block hover:opacity-90 transition-opacity shrink-0"
          style={{ background: 'linear-gradient(135deg, #4f46e5, #7c3aed)' }}
        >
      ...
      <div className="shrink-0 px-2 pb-3 pt-2" style={{ borderTop: '1px solid var(--color-border)' }}>
        <a
